...
function manifestTruth(manifest: CatalogManifest, baseType: ComponentType | undefined): { valid: boolean; reviewed: boolean; placeable: boolean; reasons: string[] } {
  ...
  if (!baseType) reasons.push(`No placeable symbol exists for ${manifest.electrical_family}`);
  ...
  const valid = reasons.every((reason) => reason.startsWith("No placeable symbol exists"));
  return { valid, reviewed, placeable: reviewed && Boolean(baseType) && valid, reasons };
}
...
export const CATALOG_PARTS: readonly CatalogPart[] = Object.entries(componentFiles).flatMap(([path, raw]) => {
  ...
  const baseType = baseTypeForFamily(manifest.electrical_family);
  const truth = manifestTruth(manifest, baseType);
  return [{
...
export class CatalogDialog {
  ...
  constructor(private readonly onPlace: (part: CatalogPart) => void) {
    ...
    this.renderList();
    if (CATALOG_PARTS[0]) void this.show(CATALOG_PARTS[0].id);
  }
  ...
  async show(partId: string): Promise<void> {
    ...
    this.detail.innerHTML = `<div class="catalog-hero"><div><span class="catalog-maker">${esc(part.manifest.manufacturer)}</span><h2>${esc(part.manifest.canonical_mpn)}</h2><p>${esc(part.manifest.description)}</p></div><span class="fidelity fidelity-large">${esc(part.manifest.fidelity_tier)}</span></div><div class="coverage-grid" aria-label="Domain coverage">${coverageMarkup(part)}</div><p>Loading package model card and validation…</p>`;
  ...
  private renderList(): void {
    ...
    this.list.querySelectorAll<HTMLButtonElement>("[data-catalog-part]").forEach((button) => button.addEventListener("click", () => void this.show(button.dataset.catalogPart!)));
  }
}
