...
export function renderSourcingStatus(
  ...
  if (!metrics) {
    return `<section class="designer-sourcing-panel" aria-labelledby="designer-sourcing-title"><header><h3 id="designer-sourcing-title">scheMAGIC Sourcing</h3>${statusChip("unavailable")}</header><p>${STATUS_COPY.unavailable}</p></section>`;
  }
