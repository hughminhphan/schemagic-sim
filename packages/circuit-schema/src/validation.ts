...
import { componentPinPoints, partByType } from "./parts";
...
function expressionIssues(
  ...
  const inspectNode = (reference: SerializedNodeReference, referencePath: string): void => {
    if (reference.kind === "runtime-node") {
      if (!reference.name.trim()) issues.push({ path: referencePath, message: "Runtime node references must not be empty" });
      return;
    }
