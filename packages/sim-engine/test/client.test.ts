...
class FakeWorker implements SimulationWorkerLike {
  ...
  ready(): void {
    this.emit({ id: 0, type: "ready", engine: "fake", engineIdentity: { ...SIMULATION_ENGINE_IDENTITY_V1 }, initMs: 1 });
  }
  ...
  succeed(request = this.posted.at(-1)): void {
    ...
    const response: WorkerResultResponse = {
      ...
      parseMs: 0.5,
      rawfileBytes: 0,
      receipt: testExecutionReceipt(request.type),
    };
    this.emit(response);
  }
...
describe("SimulationClient lifecycle", () => {
  ...
  it("keeps only the newest queued run and preserves exact result provenance", async () => {
    ...
    expect(firstResult.receipt).toEqual(testExecutionReceipt("runOpPoint"));
