import admission from "../reviewed-admission.json";
...
const BUNDLED_REVIEWED_RELEASE = deepFreeze(detachedJsonSnapshot({
...
/**
 * Browser-safe production documents: the exact release profiles plus only the
 * reviewed admission projection. Authored/planned profiles and fixtures are
 * deliberately absent while release hashes remain independently checkable.
 */
export function getBundledReviewedReleaseDocuments(): Readonly<DesignLibraryDocuments> {
  return deepFreeze(detachedJsonSnapshot(BUNDLED_REVIEWED_RELEASE)) as Readonly<DesignLibraryDocuments>;
}
