...
} from "@opencircuit/design-recipes/power-constraint-policy-engine-internal";
import {
...
export const POWER_PRODUCTION_CONSTRAINT_POLICY_V3_SCOPE_BOUNDARY =
  PRODUCTION_CONSTRAINT_POLICY_CATALOG_V3_SCOPE_BOUNDARY;

export function getInstalledPowerConstraintPolicyCatalogV3(): Readonly<ConstraintPolicyCatalogV3> {
  return detachedFrozenDesignV2Value(POWER_PRODUCTION_CONSTRAINT_POLICY_CATALOG_V3);
}

/**
 * Evaluates primary-part customization with the exact installed Power policy.
 *
 ...
 */
export function evaluatePowerPrimaryPartCustomizationWithInstalledPolicyV1(
...
/**
 * Generates a customization result with the exact installed Power policy.
 *
 * @internal The policy is installed, but `context` is caller-supplied. This
 * function does not authorize that context as the installed application
 * context and does not grant the returned result any application authority.
 */
export function generatePowerPrimaryPartCustomizedResultWithInstalledPolicyV1(
  instruction: Readonly<PrimaryPartCustomizationSidecarV1>,
  sourceGeneration: Readonly<DesignGenerationV2>,
  context: Readonly<GenerateElectricalContextV2>,
...
/**
 * Re-evaluates a customization result with the exact installed Power policy.
 *
 ...
 */
export function assertPowerPrimaryPartCustomizedResultWithInstalledPolicyV1(
...
/**
 * Evaluates a constraint decision with the exact installed Power policy.
 *
 * @internal The policy is installed, but `context` is caller-supplied. This
 * function does not authorize that context as the installed application
 * context and does not grant the decision any application authority.
 */
export function evaluatePowerConstraintDecisionWithInstalledPolicyV3(
  source: Readonly<DesignResultV2>,
  context: Readonly<ElectricalDesignContextManifestV2>,
): ConstraintDecisionV3 {
  return evaluateConstraintDecisionV3(source, context, POWER_PRODUCTION_CONSTRAINT_POLICY_CATALOG_V3);
}

/**
 * Re-evaluates a constraint decision with the exact installed Power policy.
 *
 * @internal The policy is installed, but `context` is caller-supplied. A
 * successful assertion is not installed-context or application authorization.
 */
export function assertPowerConstraintDecisionContextWithInstalledPolicyV3(
