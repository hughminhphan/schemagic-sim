...
export const STSPIN840_EVIDENCE_SOURCES = Object.freeze([
  Object.freeze({
    id: "stspin840-product-page",
    url: "https://st.com/en/motor-drivers/stspin840.html",
    fileName: "stspin840.product.html",
    contentType: "text/html",
    kind: "product_page",
  }),
  Object.freeze({
    id: "stspin840-datasheet",
    url: "https://st.com/resource/en/datasheet/stspin840.pdf",
    fileName: "stspin840.datasheet.pdf",
    contentType: "application/pdf",
    kind: "manufacturer_datasheet",
  }),
]);

const FORMAT = "schemagic-reviewed-real-evidence-capture";
...
function assertCanonicalFinalUrl(source, finalUrl) {
  ...
  if (resolved.hostname !== "st.com" && resolved.hostname !== "www.st.com") {
    throw new TypeError(`${source.id}:final_url_not_official_st`);
  }
...
function assertSourceBytes(source, bytes, contentType, finalUrl) {
  ...
  if (source.kind === "manufacturer_datasheet") {
    ...
    if (!new TextDecoder("latin1").decode(bytes).includes("STSPIN840")) throw new TypeError(`${source.id}:pdf_identity_mismatch`);
    return;
  }
  const html = new TextDecoder("utf-8", { fatal: false }).decode(bytes);
  for (const marker of ["<html", "STSPIN840", "STMicroelectronics", "TFQFPN"]) {
...
export function createSTSPIN840EvidenceReceipt(captures, capturedAt) {
  if (!Array.isArray(captures) || captures.length !== STSPIN840_EVIDENCE_SOURCES.length) {
    throw new TypeError("capture_set:exact_source_count_required");
  }
  if (!isRfc3339Instant(capturedAt)) {
    throw new TypeError("capturedAt:rfc3339_required");
  }
  const byId = new Map(captures.map((capture) => [capture.id, capture]));
  const sources = STSPIN840_EVIDENCE_SOURCES.map((source) => {
...
export async function verifySTSPIN840EvidenceReceipt(receiptPath) {
  ...
  if (!Array.isArray(parsed.sources) || parsed.sources.length !== STSPIN840_EVIDENCE_SOURCES.length) throw new TypeError("receipt:exact_source_count_required");
  const verified = [];
  for (let index = 0; index < STSPIN840_EVIDENCE_SOURCES.length; index += 1) {
    const expected = STSPIN840_EVIDENCE_SOURCES[index];
    ...
    if (bytes.byteLength !== declared.byteLength || sha256(bytes) !== declared.contentHash) throw new TypeError(`${expected.id}:exact_bytes_mismatch`);
...
export async function captureSTSPIN840Evidence({ outputDirectory, fetchImpl = globalThis.fetch, capturedAt = new Date().toISOString() }) {
  ...
  const captures = [];
  for (const source of STSPIN840_EVIDENCE_SOURCES) {
    let response;
    try {
    ...
    if (!response.ok) throw new TypeError(`${source.id}:official_http_${response.status}`);
    let bytes;
    try {
  ...
  const receipt = createSTSPIN840EvidenceReceipt(captures, capturedAt);
  await mkdir(output);
  for (const capture of captures) {
    const source = STSPIN840_EVIDENCE_SOURCES.find((candidate) => candidate.id === capture.id);
    await writeAtomically(resolve(output, source.fileName), capture.bytes);
  }
  await writeAtomically(resolve(output, "stspin840-evidence-capture.json"), `${JSON.stringify(receipt, null, 2)}\n`);
  return receipt;
}

function usage() {
  return "usage: node scripts/capture-reviewed-real-evidence.mjs capture <new-output-directory> | verify <receipt.json>";
}
...
async function main(argv) {
  ...
  const result = command === "capture"
    ? await captureSTSPIN840Evidence({ outputDirectory: path })
    : await verifySTSPIN840EvidenceReceipt(path);
