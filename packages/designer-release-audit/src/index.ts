...
function strictDefaultCandidateContractImplemented(): boolean {
  ...
  return test.includes("keeps strict generation empty until an explicit unknown-evidence opt-in")
    ...
    && exactExternalMotorInterfaceQualifiedContractImplemented(externalMotor, motorRecipeTest)
    && exactExternalMotorTvsVoltageQualifiedContractImplemented(externalMotor, motorRecipeTest)
    && test.includes('const STRICT_EMPTY_COPY = "Strict results are still in progress. Inspect an evidence-limited reference solution; unknown remains unknown."')
    && test.includes("function expectSingleStrictInspectionExit(html: string): void")
    && test.includes('expect(empty).toContain(`<p>${STRICT_EMPTY_COPY}</p>`)')
    && test.includes("expect(empty?.match(/<p\\b[^>]*>/g) ?? []).toHaveLength(1)")
    && test.includes("expect(empty?.match(/<(?:button|a)\\b/g) ?? []).toHaveLength(1)")
    && test.includes("expect(empty).not.toMatch(/<(?:button|a)[^>]*\\bdisabled\\b/)")
    ...
    && view.includes('rejection.reasonCode === "unknown_constraint_disallowed"')
    && view.includes('rejection.reasonCode === "estimated_values_disallowed"')
    && view.includes('return "Strict results are still in progress. Inspect an evidence-limited reference solution; unknown remains unknown."')
    && view.includes('<p>${escapeHtml(copy)}</p>${exactPowerInspectionAction}</section>')
    ...
    && browserTest.split('strictEmpty.locator(":scope > p")).toHaveText("Strict results are still in progress. Inspect an evidence-limited reference solution; unknown remains unknown.")').length - 1 === 3
    && browserTest.split('strictEmpty.locator("button:enabled, a[href]")).toHaveCount(1)').length - 1 === 3
